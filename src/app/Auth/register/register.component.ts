...
@Component({
  ...
  template: `
    ...
    <mat-error *ngIf="RegisterGroup.controls.login.invalid">{{getErrorMessage(RegisterGroup.controls.login)}}</mat-error>
      ...
      <mat-form-field>
        <input matInput  placeholder="{{'AUTH.REGISTER.PLACEHOLDER.EMAIL'|translate}}" type="email" formControlName="email">
        <mat-error *ngIf="RegisterGroup.controls.email.invalid">{{getErrorMessage(RegisterGroup.controls.email)}}</mat-error>
      </mat-form-field>

      <mat-form-field>
        <input matInput  placeholder="{{'AUTH.REGISTER.PLACEHOLDER.PASSWORD'|translate}}" type="password" formControlName="password">
